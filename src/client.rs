...
impl TapsilatClient {
    ...
    pub(crate) fn make_request<T>(&self, method: &str, endpoint: &str, body: Option<&T>) -> Result<ureq::Response>
    where
        T: serde::Serialize,
    {
        ...
        let mut request = match method.to_uppercase().as_str() {
            ...
            _ => return Err(TapsilatError::ConfigError(format!("Unsupported HTTP method: {}", method))),
        };

        request = request
            .set("Authorization", &format!("Bearer {}", self.config.api_key))
            .set("Content-Type", "application/json")
            .set("User-Agent", &format!("tapsilat-rust/{}", env!("CARGO_PKG_VERSION")));
